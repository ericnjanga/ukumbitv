/**
 * From Eduard Mayer
 * https://vsxed.github.io/play-store-menu/
*/

$(document).ready(function () {
	var link = $('.GP-menu .play-menu li a'),
	...
	link.each(function() {
		$(this).on('click', function() {
			...
			link.not($(this)).parent().addClass('GP-hide');
	...
	container.on('mouseleave', function() {
		$('.GP-hide').css({'left': left, 'opacity' : 0, 'transition' : '', '-webkit-transition' : '', '-moz-transition' : ''});
		$('.sub-menu').removeClass('index');
	})

	// Show the menu when you hover the trigger 
	expander.on('mouseover', function() {
		$('.GP-hide').css({'left': 0, 'opacity' : 1, 'transition' : '', '-webkit-transition' : '', '-moz-transition' : ''});
