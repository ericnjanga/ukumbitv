...
@section('content')
  <div class="page-auth" ng-app="validationApp" ng-controller="mainController">  
    ...
    <form  action="{{ url('/register') }}" method="POST" autocomplete="off" novalidate>
      ...
      <div class="form-group" ng-class="{ 'has-error' : (loginForm.password.$valid && loginForm.confirmPassword.$dirty &&loginForm.confirmPassword.$error.passwordMatch) }">
        ...
        <p ng-show="(loginForm.password.$valid && loginForm.confirmPassword.$dirty &&loginForm.confirmPassword.$error.passwordMatch)" class="help-block">Password doesn't match.</p> 
      </div>
      ...
      <button type="submit" class="btn btn-block btn-cta1b btn-lg" ng-disabled="loginForm.$invalid">{{trans('messages.auth_signup')}}</button>
    </form>
