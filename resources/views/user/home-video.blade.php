    ...
    <div class="row">
      <ul id="og-grid" class="og-grid video-content list-unstyled clearfix">
        @foreach($videos as $video)
          ...
          -->
            <a href="{{url('/')}}/watch/{{$video->watchid}}" data-largesrc="{{$video->imgPreview}}" data-title="{{$video->title}}" data-description="{{$video->description}}">
                <img class="og-tmb1" src="{{$video->imgSmall1}}" alt="{{$video->title}}"/>
            </a> 
          </li>
        @endforeach 
      </ul>
